package com.sulake.habbo.navigator.roomsettings
{
   ...
   public class RoomSettingsCtrl
   {
      ...
      private var var_1123:TextFieldManager;
      ...
      private var var_362:IRoomSettingsCtrlOwner;
      ...
      {
         close();
         this.var_362.roomSettingsRefreshNeeded();
      }
      ...
      private function populateForm() : void
      {
         ...
         {
            case RoomSettingsData.const_146:
         ...
         setTag(var_853,_loc1_.tags[0]);
         setTag(var_854,_loc1_.tags[1]);
         if(var_1125)
         {
            if(_loc1_.allowPets)
            {
      ...
      private function onConfirmRoomDelete(param1:WindowMouseEvent, param2:int) : void
      {
         ...
         close();
         var_362.roomSettingsRefreshNeeded();
         ...
         _navigator.toolbar.events.dispatchEvent(new HabboToolbarSetIconEvent(HabboToolbarSetIconEvent.const_142,HabboToolbarIconEnum.ROOMINFO));
      }
      ...
      {
         Logger.log("[RoomSettingsCtrl.onDeleteButtonClick] " + var_363);
         _navigator.registerParameter("navigator.roomsettings.deleteroom.confirm.message","room_name",var_83.name);
         var _loc2_:ConfirmDialogView = new ConfirmDialogView(IFrameWindow(_navigator.getXmlWindow("ros_room_delete_confirm")),this,onConfirmRoomDelete,[var_363]);
         _loc2_.show();
      }
      ...
      {
         return IWindowContainer(_navigator.windowManager.createWindow(param1,"",HabboWindowType.const_39,HabboWindowStyle.const_20,HabboWindowParam.const_49,new Rectangle(0,0,100,20)));
      }
      ...
      public function refresh(param1:IWindowContainer) : void
      {
         ...
         var_1383.visible = _inRoom;
         var_1712.visible = var_431;
         var_1711.visible = !var_431;
         if(var_431)
         {
         ...
         else
         {
            ...
            var_1119.visible = _inRoom;
            _navigator.thumbRenderer.refreshThumbnail(IWindowContainer(var_1119.findChildByName("picframe")),_navigator.data.enteredGuestRoom.thumbnail,false);
            var_928.height = Util.getLowestPoint(this.var_928) + 4;
         }
         var_1385.text = !!var_431 ? "${navigator.roomsettings.tobasicsettings}" : "${navigator.roomsettings.toadvancedsettings}";
         Util.moveChildrenToColumn(_window,[var_1386.name,var_1119.name,var_928.name,var_927.name,var_1122.name,var_1383.name],0,0);
      ...
      {
         this.var_363 = param1;
         _navigator.send(new GetRoomSettingsMessageComposer(var_363));
         if(var_930 != null)
         {
            var_930.visible = false;
         }
      }
      ...
      private function onSaveButtonClick(param1:WindowMouseEvent) : void
      {
         ...
         switch(_loc4_.name)
         {
            case "doormode_doorbell":
               _loc2_.doorMode = RoomSettingsFlatInfo.const_146;
               break;
            case "doormode_password":
               _loc2_.doorMode = RoomSettingsFlatInfo.const_101;
               break;
            default:
               _loc2_.doorMode = RoomSettingsFlatInfo.const_352;
         }
         if(_loc2_.doorMode == RoomSettingsFlatInfo.const_101)
         {
            _loc8_ = var_929.getText();
            _loc9_ = var_1123.getText();
            if(_loc8_ != _loc9_)
            {
               this.var_929.clearErrors();
               this.var_1123.displayError("${navigator.roomsettings.invalidconfirm}");
               return;
            }
            if(_loc8_ != "")
            {
         ...
         _loc2_.allowPets = var_1125.isSelected;
         _loc2_.allowFoodConsume = var_1124.isSelected;
         _loc2_.allowWalkThrough = var_1120.isSelected;
         _loc2_.tags = new Array();
         addTag(var_853,_loc2_.tags);
         addTag(var_854,_loc2_.tags);
         this.clearErrors();
      ...
      private function onRemoveAllFlatCtrlsClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         _navigator.send(new RemoveAllRightsMessageComposer(this.var_363));
      }
      
      public function onRoomSettingsSaved(param1:int) : void
      {
         if(param1 != this.var_363 || var_926 < 1)
         {
            return;
         }
         close();
      ...
      public function onRoomSettingsSaveError(param1:int, param2:int, param3:String) : void
      {
         ...
         var_926 = 0;
         if(param2 == RoomSettingsSaveErrorMessageParser.const_1206)
         {
         ...
         {
            setTagError(this.var_853,param3,"${navigator.roomsettings.unacceptablewords}");
            setTagError(this.var_854,param3,"${navigator.roomsettings.unacceptablewords}");
         }
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1345)
         {
            setTagError(this.var_853,param3,"${navigator.roomsettings.nonuserchoosabletag}");
            setTagError(this.var_854,param3,"${navigator.roomsettings.nonuserchoosabletag}");
         }
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1141)
         {
            this.var_929.displayError("${navigator.roomsettings.passwordismandatory}");
         }
         else
         {
      ...
      private function prepareWindow(param1:IWindowContainer) : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_623,onDoorModePasswordSelect);
         _loc2_.addEventListener(WindowEvent.const_770,onDoorModePasswordUnselect);
         ...
         this.var_853 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("tag1")),30);
         this.var_854 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("tag2")),30);
         this.var_929 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("password")),30);
         this.var_1123 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("password_confirm")),30);
         this.var_1125 = ICheckBoxWindow(_window.findChildByName("allow_pets_checkbox"));
         this.var_1124 = ICheckBoxWindow(_window.findChildByName("allow_foodconsume_checkbox"));
         this.var_1120 = ICheckBoxWindow(_window.findChildByName("allow_walk_through_checkbox"));
         var_1386 = IWindowContainer(_window.findChildByName("header_container"));
         var_1119 = IWindowContainer(_window.findChildByName("thumbnail_container"));
         var_928 = IWindowContainer(_window.findChildByName("basic_settings_container"));
         var_927 = IWindowContainer(_window.findChildByName("advanced_settings_container"));
         var_1383 = IWindowContainer(_window.findChildByName("switch_view_container"));
         ...
         var_930 = IWindowContainer(_window.findChildByName("password_container"));
         var_1711 = ITextWindow(_window.findChildByName("basic_caption"));
         var_1712 = ITextWindow(_window.findChildByName("advanced_caption"));
