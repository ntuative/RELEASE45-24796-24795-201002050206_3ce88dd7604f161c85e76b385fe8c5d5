package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_398 >= var_284.length)
         {
            return "";
         }
         var petColor:IPetColor = var_284[var_398] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_560[var_1498] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_398 = param1.index;
         if(var_398 < 0 || var_398 > var_284.length)
         {
            var_398 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_541.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         var_823 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_284 = [];
         var_560 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_823);
               if(_loc11_)
               {
                  var_560 = _loc11_.breeds;
                  var_284 = _loc11_.colors;
                  if(var_284 == null)
                  {
                     var_284 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_560.length)
         {
            _loc12_ = var_560[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_823,_loc12_.id),getRaceLocalizationKey(var_823,_loc12_.id)));
            _loc6_++;
         }
         var_541.add(var_823,_loc5_);
         ...
         _loc5_ = var_541.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_284)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_541.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_398 >= 0 && var_398 < var_284.length)
         {
            _loc7_ = var_284[var_398] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_560[var_1498] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_823,_loc3_.id,_loc2_,AvatarScaleType.const_34);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_280,AvatarAction.const_650);
