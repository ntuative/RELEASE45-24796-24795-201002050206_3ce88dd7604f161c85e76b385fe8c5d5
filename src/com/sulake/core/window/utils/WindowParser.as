package com.sulake.core.window.utils
{
   ...
   public class WindowParser implements IWindowParser
   {
      ...
      public static const const_1472:String = "_INCLUDE";
      
      public static const const_914:String = "_EXCLUDE";
      
      public static const const_1561:String = "_TEMP";
      ...
      protected var var_887:Dictionary;
      
      protected var var_1324:Map;
      
      protected var _context:IWindowContext;
      
      protected var var_1323:Dictionary;
      
      protected var var_1059:Dictionary;
      ...
      public function WindowParser(param1:IWindowContext)
      {
         ...
         TypeCodeTable.fillTables(var_1059,var_1060);
         var_887 = new Dictionary();
         var_1323 = new Dictionary();
         ParamCodeTable.fillTables(var_887,var_1323);
         var_1324 = new Map();
      }
      ...
      private function buildBitmapFilter(param1:XML) : BitmapFilter
      {
         ...
         switch(_loc2_)
         {
            case "DropShadowFilter":
               _loc3_ = new DropShadowFilter(Number(parseAttribute(param1,"distance",null,"0")),Number(parseAttribute(param1,"angle",null,"45")),uint(parseAttribute(param1,"color",null,"0")),Number(parseAttribute(param1,"alpha",null,"1")),Number(parseAttribute(param1,"blurX",null,"0")),Number(parseAttribute(param1,"blurY",null,"0")),Number(parseAttribute(param1,"strength",null,"1")),int(parseAttribute(param1,"quality",null,"1")),Boolean(parseAttribute(param1,"inner",null,"false") == "com.sulake.habbo.room.object.visualization.room.publicroom.rasterizer"),Boolean(parseAttribute(param1,"knockout",null,"false") == "com.sulake.habbo.room.object.visualization.room.publicroom.rasterizer"),Boolean(parseAttribute(param1,"hideObject",null,"false") == "com.sulake.habbo.room.object.visualization.room.publicroom.rasterizer"));
         }
         return _loc3_;
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            for(_loc1_ in var_1059)
            {
               delete var_1059[_loc1_];
            }
            for(_loc1_ in var_1060)
            {
               delete var_1060[_loc1_];
            }
            for(_loc1_ in var_887)
            {
               delete var_887[_loc1_];
            }
      ...
      private function parseSingleWindowEntity(param1:XML, param2:WindowController, param3:Map) : IWindow
      {
         ...
         background = parseAttribute(xml,"background",variables,background.toString()) == "com.sulake.habbo.room.object.visualization.room.publicroom.rasterizer";
         treshold = uint(parseAttribute(xml,"treshold",variables,treshold.toString()));
         visible = parseAttribute(xml,"visible",variables,visible.toString()) == "com.sulake.habbo.room.object.visualization.room.publicroom.rasterizer";
         clipping = parseAttribute(xml,"clipping",variables,clipping.toString()) == "com.sulake.habbo.room.object.visualization.room.publicroom.rasterizer";
            ...
            if(parent != null)
            {
               if(parent is IIterable)
               {
                  if(window.x != rect.x || window.y != rect.y || window.width != rect.width || window.height != rect.height)
                  {
                     ...
                     if((params & 0) == WindowParam.const_139)
                     {
                        window.y = rect.y;
                     }
                  }
         ...
         if(_loc7_ is IIterable)
         {
            ...
            if(_loc9_ > 0)
            {
               ...
               while(_loc12_ < _loc9_)
               {
                  ...
                  if(_loc10_.tags.indexOf(WindowParser.const_914) == -1)
                  {
